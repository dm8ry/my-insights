...
<head>
  ...
  <?php
    ...
    // sql_4 - registrations weekly 
      ...
      $sql_4 = "select * from 
					(select to_char(created_at, 'YY WW') week, count(1) n_of_regs 
					   from fct_users 
					  where created_at >= date_trunc('day', current_timestamp - interval '100 days') 
					  group by to_char(created_at, 'YY WW') 
					  order by to_char(created_at, 'YY WW') 
					  desc limit 9) M 
					  order by 1";
    ...
    // sql_30 - FTDs # Weekly
      ...
      $sql_30 = "select to_char(ftd_date, 'YY WW') the_dt, count(1) n_of_ftds
				from fct_24h_ftds
				where ftd_date>=  date_trunc('week', current_timestamp) - interval '6 weeks'
				group by to_char(ftd_date, 'YY WW')
				order by to_char(ftd_date, 'YY WW')
				";
    ...
    // sql_42 - Deposits recent Weeks
      ...
      $sql_42 = "select to_char(deposit_dttime, 'YY WW') the_dt, 
						round(sum(case when dep_currency = 'USD' then dep_profit else 0 end)) dep_usd,
						round(sum(case when dep_currency = 'EUR' then dep_profit else 0 end)) dep_eur,
						round(sum(case when dep_currency = 'GBP' then dep_profit else 0 end)) dep_gbp
				from fct_deposits
				where deposit_dttime >= date_trunc('week', current_timestamp) - interval '7 weeks'
				group by to_char(deposit_dttime, 'YY WW')
				order by to_char(deposit_dttime, 'YY WW')";
    ...
    // sql_93 - Trades # recent Weeks
      ...
      $sql_93 = "select to_char(close_time, 'YY WW') the_dt, 
	              count(1) n_of_trades,				  
				  round(sum(trade_close_volume_usd)) tr_close_vol_usd, 
				  round(sum(trade_close_profit_usd)) tr_close_prof_usd,
				  round(sum(trade_revenue_usd)) tr_rev_usd
				...
				group by to_char(close_time, 'YY WW')
				order by to_char(close_time, 'YY WW')  ";
	...
	$sql_83 =  "select to_char(current_sign_in_at,  'YY WW')  the_dt, count(1) n_online_users 
	           from fct_users where current_sign_in_at >= date_trunc('week', current_timestamp - interval '6 weeks')
			   group by to_char(current_sign_in_at,  'YY WW') 
			   order by  to_char(current_sign_in_at,  'YY WW')";
